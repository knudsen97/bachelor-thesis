 ...
 * @param _cameraImage A cv::Mat of the current map
*/
void drawEndPoints(int _i, planner &_plan, argos::CPrototypeEntity* _pcObject, 
                    std::vector<argos::CVector3> _validPushPoints, argos::CVector3 _boxGoal, cv::Mat &_cameraImage)
{
    argos::CVector3 robotEndPoint(0,0,0);
 ...
 * @param _debug Parameter to write things to the log
*/
bool serverWaitState(argos::CPrototypeEntity *pcObject, argos::CVector3 _boxGoal, int _clientcount, std::vector<protocol> &_clientConnections, double threshold, bool _debug = false)
{
    bool _inRange;
    argos::CVector3 boxOrigin = pcObject->GetEmbodiedEntity().GetOriginAnchor().Position;
    argos::Real distanceToGoal = argos::Distance(_boxGoal, boxOrigin);
